...
---
Analyze the current changes and generate a conventional commit message. **Output the
message only -- do not commit.**
...
4. Run `gh issue list --state open --limit 50 --json number,title` and add `Closes #NNN`
   for each issue the diff fully resolves
5. Generate a message: `<type>(<scope>): <description>` + body with bullet points
   summarizing key changes
6. Output in a code block for the user to copy
